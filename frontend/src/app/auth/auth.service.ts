...
export class AuthService {
  ...
  private apiUrl = 'http://localhost:3000/recipes'; // Muuta tarvittaessa reitti omasi mukaiseksi
  ...
  syncUserWithBackend() {
    ...
    combineLatest([
      ...
      if (isAuthenticated && userData) {
        ...
        this.http.post(`${this.apiUrl}/sync`, syncData).subscribe({
          next: () => console.log('Käyttäjä synkronoitu tietokantaan onnistuneesti'),
          error: (err) => console.error('Virhe käyttäjän synkronoinnissa:', err)
        });
      }
    });
  }
