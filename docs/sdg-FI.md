...
Kestävä kehitys on tärkeää minulle ja tiimilleni koska kyse on elämänlaadumme turvaamisesta ja koska haluamme säästää mahdollisimman paljon maapallomme rajattuja resursseja.
...
Ruokahävikkiä vähennetään antamalla ihmisille helppokäyttöinen sovellus herkullisten reseptien löytämiseen.
Kulttuuriperinnön suojeleminen antamalla ihmisille tilaisuus jakaa edellisten sukupolvien reseptejä.
...
Minimoidaan energiankulutus pitämällä aws palvelut yllä vain tarvittavana aikana, tämän lisäksi minimoidaan tietokantakyselyt ja tekoälyn käyttö.
...
Sovellus voi muokata kuluttajien raaka aineiden hankkimista ja käyttämistä suuntaan tai toiseen, toisaalta jos kuluttaja lähtee hakemaan reseptin takia aineita kaupasta autolla, saattaa sovelluksemme lisätä kulutusta mutta jos taas kuluttaja käyttää edelliseen reseptiin hankkimiaan aineita, saattaa kulutus laskea.
...
Sovelluksen lähdekoodi ja kehitysprosessi on dokumentoitu ja sovellus vaatii muutamaa ulkoista kirjastoa ja järjestelmää kuten expressin angularin ja npm sen kirjastojen kanssa. Tämän sovellus vaatii AWS palveluita toimiakseen koska hostaaminen on hoidettu sen kautta.
...
Sovellus ei tee oletuksia käyttäjistä ja noudatetaan WCAG 2.1 AA kriteerejä. Tämän lisäksi sovelluksen käyttö on ilmaista. Sovellus on aluksi saatavilla vain suomeksi mutta lokalisaatio muille kielille on toteutettavissa.
...
Sovellus parantaa sosiaalisia suhteita, terveyttä ja mielialaa tarjoamalla käyttäjille herkullisia ja monipuolisia reseptejä.
...
Sovellus ei suoraan vaikuta häiritsevään käyttäytymiseen ja kommentit resepteihin moderoidaan kehittäjien toimesta.
...
Noudatetaan ketterää kehitystä scrum mallia hyödyntäen ja otetaan kehittäjien hyvinvointi huomioon sopivilla työajoilla.
